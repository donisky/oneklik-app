...
export default function ReportPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setLoading(true);
    setTimeout(() => {
      toast.success('Laporan Anda telah terkirim. Tim kami akan segera menindaklanjuti.');
      setEmail('');
      setIssue('');
      setDescription('');
      setLoading(false);
        ...
        <div className="bg-white/80 backdrop-blur-sm p-8 md:p-12 rounded-3xl border border-slate-200 shadow-xl shadow-slate-100/50">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select value={issue} onChange={(e) => setIssue(e.target.value)} className="w-full border border-slate-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none">
            ...
            <div>
              ...
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} className="w-full border border-slate-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" rows={5} placeholder="Jelaskan secara detail masalah yang Anda temui..." />
            </div>
            <button type="submit" disabled={loading} className="w-full py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl font-medium transition-colors flex items-center justify-center gap-2 disabled:opacity-50">
