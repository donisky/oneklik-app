...
export default function FileQRClient() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (session) {
          // Ambil data user dari tabel 'users' (untuk cek status premium & avatar)
  ...
  // --- Helper untuk avatar di tengah QR ---
      ...
      <div className="relative inline-block">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-full border-2 border-white shadow-md overflow-hidden w-10 h-10 flex items-center justify-center">
          {avatarUrl ? (
            ...
            <span className="text-xs font-bold text-purple-600 bg-purple-50 w-full h-full flex items-center justify-center">
              {initials}
            </span>
          )}
        </div>
  ...
  // --- 4. KONTEN UTAMA (SUDAH LOGIN) ---
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-purple-50 flex flex-col overflow-y-auto">
