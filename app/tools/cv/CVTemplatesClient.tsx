...
export default function CVTemplatesClient() {
  ...
  useEffect(() => {
    if (!loading && !session) {
      // Redirect ke halaman login, dengan next parameter agar kembali ke sini setelah login
  ...
  // --- JIKA BELUM LOGIN, redirect sudah dilakukan, jadi tidak perlu render apa pun ---
  // (Tapi karena redirect terjadi, komponen ini tidak akan sempat dirender)
  // Untuk keamanan, kita tetap return null agar tidak ada flash.
  if (!session) {
    return null;
  }
